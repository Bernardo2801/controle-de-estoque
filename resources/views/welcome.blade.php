        ...
        <table class="table">
            ...
            <tbody>
                @foreach($stocks as $stock)
                    <tr>
                        <td><a class="text-decoration-none" href="/stocks/{{ $stock->id }}">{{ $stock->product }}</a></td>
                        <td><a class="text-decoration-none" href="/stocks/{{ $stock->id }}">{{ $stock->quantity }}</a></td>
                        <td><a class="text-decoration-none" href="/stocks/{{ $stock->id }}">R${{ number_format($stock->value, 2, ',', '.') }}</a></td>
                        <td>
                            ...
                            <form action="/stocks/{{ $stock->id}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger text-bg-danger">Deletar</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
